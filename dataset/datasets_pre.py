...
class SpeechDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
        ...
        self.class_id = self.load_class_id(split_dir,200) #len(self.filenames))
    ...
    def __getitem__(self, index):
        ...
        if cfg.SPEECH.style == 'WAV':
            ...
            if  self.data_dir.find('Flickr8k') != -1:
                ...
                audio_file = '%s/audio_mel_arabic/%s.npy' % (data_dir, key) 
            ...
            audios = np.load(audio_file,allow_pickle=True)
            if len(audios)<10:
              print("####### file name",len(audios) , audio_file)
