...
def train(Models,train_loader, test_loader, args):    
    ...
    if epoch != 0:
        ...
        if cfg.DATASET_NAME == 'birds' or cfg.DATASET_NAME == 'flowers':
            class_model.load_state_dict(torch.load("%s/models/class_model_%d.pth" % (exp_dir,epoch)))        
    ...
    while epoch<=cfg.TRAIN.MAX_EPOCH:
        epoch += 1
        print(">>>> Hello we are in epoch",epoch)
        ...
        if epoch % 5 ==0:
            ...
            if cfg.DATASET_NAME == 'birds' or cfg.DATASET_NAME == 'flowers':
                torch.save(class_model.state_dict(),
                    "%s/models/class_model_%d.pth" % (exp_dir,epoch))
            if avg_acc > best_acc:
                best_epoch = epoch
                best_acc = avg_acc
